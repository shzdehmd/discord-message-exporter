 ...
 * @param {string} targetDirectory - Absolute path for the final hashed file.
 * @param {string} baseExportDir - Absolute path to the base export dir for relative path calc.
...
async function downloadAndDeduplicateFile(url, targetDirectory, baseExportDir, forcedExtension = null) {
    ...
    if (forcedExtension) {
        /* ... keep extension logic ... */ derivedExtension = forcedExtension.startsWith('.')
            ? forcedExtension
            : `.${forcedExtension}`;
    } else {
        try {
            /* ... keep extension logic ... */ const p = new URL(url).pathname;
    ...
    try {
        ...
        const response = await axiosInstance.get(url, { responseType: 'stream', timeout: 60000 });
        ...
        try {
            ...
            fileExisted = true; // File EXISTS - Deduplication case
            ...
            try {
                // --- Attempt Rename First (fastest) ---
                await fs.rename(tempFilePath, finalAbsolutePath);
                tempFilePath = null; // Mark as moved
                logger.logInfo(` -> Successfully renamed temp file to final path.`);
            } catch (renameError) {
                // --- If rename fails with EXDEV, fallback to Copy + Unlink ---
                if (renameError.code === 'EXDEV') {
                    logger.logWarn(` -> Rename failed (cross-device). Falling back to copy for ${finalFileName}.`);
                    await fs.copyFile(tempFilePath, finalAbsolutePath);
                    // Verify copy? Optional, could hash the copied file again for paranoia
                    logger.logInfo(` -> Successfully copied temp file to final path.`);
                    // Temp file still exists, will be deleted in finally block
                } else {
                    // If rename failed for another reason, re-throw the error
                    logger.logError(` -> Rename failed for unexpected reason: ${renameError.code}`, renameError);
                    throw renameError;
                }
            }
        }
        ...
        let thrownError = error; // ... (keep error standardization logic) ...
        ...
        if (tempDir) {
            try {
                // Ensure tempFilePath wasn't set to null (meaning it was successfully moved)
                // Although fs.rm should handle non-existent files gracefully with force: true
