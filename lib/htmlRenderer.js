...
function discordMessageToHtmlSingle(message, isGrouped = false) {
    ...
    if ((message.type === 19 || message.message_reference) && message.referenced_message) {
        ...
        if (repliedAuthor?.id) {
            ...
            // Use getAvatarUrl for the replied author's avatar as well
            const repliedAvatarUrl = getAvatarUrl(repliedAuthor, 32); // Smaller size for reply context

            // Generate content preview
            let previewText = '[Attachment or Embed]'; // Default preview
            if (repliedMsg.content) {
                // Basic text preview, limit length, strip markdown/mentions for simplicity
                previewText = repliedMsg.content
                    .replace(/<[@#][!&]?\d+>/g, (m) => `@mention`) // Simplify mentions
                    .replace(/[*_~`|]/g, '') // Strip basic markdown chars
                    .replace(/EMOTE_MARKER:[^|]+\|([^\]\s]+)/g, ':$1:') // Show :emote_name: approx
                    .replace(/\s+/g, ' '); // Collapse whitespace
                if (previewText.length > 80) previewText = previewText.substring(0, 77) + '...';
                previewText = sanitize(previewText); // Sanitize the result
            ...
            html += `<span class="reply-content-preview">${previewText}</span>`;
        } else {
            html += `<span class="reply-deleted">[Original Message Deleted or Author Unknown]</span>`;
        }
